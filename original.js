...
function htmlToFalco(html) {
            ...
            switch (key) {
                ...
                default:
                    ...
                    if (key.startsWith('data-')) {
                        ...
                        props.push(`Attr.custom ("data-${dataKey}") "${escapeString(value)}"`);
                    } else {
                        // Generic attribute
                        props.push(`Attr.custom "${key}" "${escapeString(value)}"`);
                    }
                    break;
            }
        }
    ...
    function processDOM(dom) {
        const body = dom.children.map(child => processNode(child)).filter(node => node !== '').join('\n')
        return body;
    }
